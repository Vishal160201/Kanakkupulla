...
const filePath = 'src/components/dashboard/BookingDetailsModal.tsx';
let content = fs.readFileSync(filePath, 'utf-8');

// 1. Remove isEditing state and effects
content = content.replace(/const \[isEditing, setIsEditing\] = useState\(false\);\n  const \[editData, setEditData\] = useState<any>\(\{\}\);\n  const \[formErrors, setFormErrors\] = useState<Record<string, boolean>>\(\{\}\);\n  const \[installments, setInstallments\] = useState<\{amount: string, date: string\}\[\]>\(\[\]\);\n  const timeInputRef = useRef<HTMLInputElement>\(null\);/g, '');

content = content.replace(/const handleEditToggle = \(\) => \{[\s\S]*?\n  \};\n\n  useEffect\(\(\) => \{[\s\S]*?return \(\) => \{ if \(fp\) fp\.destroy\(\); \};\n  \}, \[isEditing\]\);\n\n  const handleSaveEdit = async \(\) => \{[\s\S]*?\n  \};/g, '');

// 2. Replace the main grid area and Custom Dynamic Fields
const newMainContent = `                {/* 3 Main Columns for Dynamic Sections */}
...
content = content.replace(/\{\/\* 3 Main Columns \*\/\}\n\s*<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">[\s\S]*?\{\/\* Custom Dynamic Fields \*\/\}\n[\s\S]*?\{\/\* Package & Payment \*\/\}/g, newMainContent + '\n\n                {/* Package & Payment */}');

// 3. Remove inline edit toggle buttons from UI
content = content.replace(/\{isEditing \? \([\s\S]*?\) : \([\s\S]*?\)\}/g, (match) => {
  // Extract the non-editing branch
  const parts = match.split(') : (');
  if (parts.length === 2) {
    let result = parts[1].trim();
    if (result.endsWith('}')) result = result.slice(0, -1).trim();
    // Some logic might be broken, actually the script above replaces the ENTIRE 3 columns block which had 90% of these!
    return result;
  }
  return match;
});

// Remove top level editing title logic
content = content.replace(/\{isEditing \? \([\s\S]*?<input[\s\S]*?\) : \([\s\S]*?<h1[\s\S]*?\{booking\.title[\s\S]*?<\/h1>[\s\S]*?\)\}/g, `<h1 className="text-[2.5rem] font-black text-[#0B1E40] leading-none tracking-tight mb-2">
                           {booking.title || 'Untitled Booking'}
                        </h1>`);

// Remove editing category dropdown
content = content.replace(/\{isEditing \? \([\s\S]*?<Select value=\{editData\.category\}[\s\S]*?\) : \([\s\S]*?<span className="text-\[#0B1E40\] font-bold text-\[0\.95rem\] leading-tight">\{booking\.category[\s\S]*?\)\}/g, `<span className="text-[#0B1E40] font-bold text-[0.95rem] leading-tight">{booking.category || 'Uncategorized'}</span>`);

// Remove bottom editing footer
content = content.replace(/\{isEditing \? \([\s\S]*?Cancel Edit[\s\S]*?Save Changes[\s\S]*?\) : \([\s\S]*?Edit Booking[\s\S]*?Close[\s\S]*?\)\}/g, `<>
                   <button onClick={() => {
                        setIsClosing(true);
                        setTimeout(() => {
...
console.log('Successfully patched BookingDetailsModal.tsx');
