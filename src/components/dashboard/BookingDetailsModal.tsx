  ...
  const handleEditToggle = () => {
    if (!isEditing) {
      setEditData({
         ...
         phone: (booking as any)?.client?.phone || booking?.phone || '',
         email: (booking as any)?.client?.email || booking?.email || '',
         packageName: booking?.packageName || booking?.customData?.fld_b_package_name || '',
         inclusions: Array.isArray(booking?.inclusions) ? booking.inclusions.join(', ') : '',
         package: (booking as any)?.order?.package?.toString() || booking?.customData?.fld_b_package?.toString() || '',
         advance: (booking as any)?.order?.advance?.toString() || booking?.customData?.fld_b_advance?.toString() || '',
         due: (booking as any)?.order?.due?.toString() || '',
